...
async def test_shutdown_preserves_growth_observation_blocked_behind_stalled_upload(
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(stalled)) as client:
        ...
        try:
            ...
            # Finish the unrelated empty scope's cold SQLite setup before
            # waiting for the growth send that establishes the shutdown race.
            await runtime._scope_runtime(TelemetryScope.RELIABILITY)
            await runtime.start()
