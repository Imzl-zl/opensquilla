...
def test_effect_cd_prefix_accepts_only_exact_workspace(tmp_path, action, compact):
    import shlex

    workspace = tmp_path / "workspace with spaces"
    workspace.mkdir()
    join = "&&" if compact else " && "
    command = f"cd {shlex.quote(str(workspace))}{join}python3 effect_fixture.py {action}"
    arguments = {"command": command, "workdir": str(tmp_path)}
    assert live._effect_command(arguments, workspace) == action
    arguments["command"] = f"cd .{join}python3 effect_fixture.py {action}"
    assert live._effect_command(arguments, workspace) is None
    arguments["workdir"] = str(workspace)
    assert live._effect_command(arguments, workspace) == action


@pytest.mark.parametrize("command", [
    "cd .. && python3 effect_fixture.py record",
    "cd .; python3 effect_fixture.py record",
    "cd . || python3 effect_fixture.py record",
    "cd . && python3 effect_fixture.py record && echo RECORDED",
    "cd . && python3 effect_fixture.py record || true",
    "cd . && python3 effect_fixture.py record; true",
    "cd . && python3 effect_fixture.py record > receipt.txt",
    "cd . && python3 effect_fixture.py record 2>&1",
    "cd . && python3 effect_fixture.py record &",
    "cd . && python3 effect_fixture.py record extra",
    "cd . && python3 effect_fixture.py $ACTION",
    "cd . && python3 effect_fixture.py `echo record`",
    "cd . && python3 effect_fixture.py record\n",
    "cd . && python3 effect_fixture.py record\r",
    "cd . && python3 other.py record",
    "cd . && python3 effect_fixture.py record # hidden",
])
def test_effect_cd_prefix_rejects_commands_that_obscure_execution(tmp_path, command):
    assert live._effect_command({"command": command}, tmp_path) is None


async def test_effect_cd_evidence_preserves_turn_response_and_replay_checks(tmp_path):
    path = tmp_path / "effect_fixture.py"
    path.write_text(live.EFFECT_FIXTURE)
    fixture = path.read_bytes()
    assert await _fixture(tmp_path, "record") == 0
    records = _rows("implementation", "record")
    records[0]["payload"]["arguments"]["command"] = (
        "cd . && python3 effect_fixture.py record"
    )
    proof = live.effect_evidence(records, tmp_path, fixture, "implementation")
    assert proof["record_succeeded_on_original_turn"] and proof["receipt_exactly_once"]
    assert proof["record_requests"] == 1 and proof["opaque_exec_requests"] == 0
    assert not live.effect_evidence(records, tmp_path, fixture, "other")[
        "record_succeeded_on_original_turn"
    ]
    records[1]["payload"]["result"] = "exit_code=1\nRECORDED\n"
    assert not live.effect_evidence(records, tmp_path, fixture, "implementation")[
        "record_succeeded_on_original_turn"
    ]
    records[1]["payload"]["result"] = "exit_code=0\nRECORDED\n"
    records.extend(_rows("implementation", "record", 3))
    proof = live.effect_evidence(records, tmp_path, fixture, "implementation")
    assert proof["record_requests"] == 2 and not proof["record_succeeded_on_original_turn"]
